...
function Nav({
	...
	const handleNextClick = React.useCallback(() => {
		...
		setDisplayYears((prev) => {
			...
			const updated = { from: prev.from + span, to: prev.to + span };
			onNextClick?.(new Date(updated.from, 0, 1));
			return updated;
		});
	}, [navView, nextMonth, goToMonth, onNextClick]);
	...
	const handlePreviousClick = React.useCallback(() => {
		...
		setDisplayYears((prev) => {
			const span = prev.to - prev.from + 1;
			const updated = { from: prev.from - span, to: prev.to - span };
			onPrevClick?.(new Date(updated.from, 0, 1));
			return updated;
		});
	}, [navView, previousMonth, goToMonth, onPrevClick]);
	return (
